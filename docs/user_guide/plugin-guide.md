...
| `wiki-publish` | Runs `worklog wiki-plan` (frozen-guard + render-hash skip), then publishes via `docs/.index/publish-manifest.json` when present (Home, Sidebar, indexes, per-ticket/release/PR artifact pages, banners) and the `.work/published.jsonl` ledger; strips YAML frontmatter for GitHub Wiki |
...
| `design-docs` | Generates/syncs the live `current_*` design doc + code walkthrough under `docs/designs/`; a release freeze is a short tag+hash+delta note, not a full dated copy. Existing full copies stay frozen. Runs in background agents at release time. Must run `worklog doc-verify` and clear every fabricated citation before reporting done — that check is why the pair's line numbers can be trusted |
