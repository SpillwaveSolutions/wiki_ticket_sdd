...
| `wiki-publish` | Publishes via `docs/.index/publish-manifest.json` when present (Home, Sidebar, indexes, per-ticket/release/PR artifact pages, banners) and the `.work/published.jsonl` ledger; strips YAML frontmatter for GitHub Wiki |
