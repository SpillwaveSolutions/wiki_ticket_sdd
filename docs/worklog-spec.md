...
- **Idempotency** — the rule that moved here intact from the adapter contract, because it was always the single most important one: **push must be idempotent, keyed by item ULID** (invariant §15.5). Every ticket body embeds a marker line, `<!-- worklog:<ULID> -->`; before creating, the skill searches the tracker for that marker and updates the hit instead of creating. A retried push must find, never duplicate — without this, a retried subagent opens duplicate tickets. *(0.24.11, #412)* The dispatcher implements this as the third source of `remembered_key`: the marker-to-key map that `observe_remote` builds from the listing it already makes, after the log's `external.key` and the per-clone `last_pushed_key`. It is bounded by what the adapter's pull returns (a capped or lagging listing can still miss), so `dedupe` remains the backstop.
