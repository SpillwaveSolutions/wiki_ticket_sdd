...
tag: v0.12.0
git_hash: ee38438fec2f855836e65c9a95979c321efcc3d2
branch: main
roadmap_snapshot: docs/roadmap/2026-07-21_v0.12.0-release.md
merged_in: 26b631fdc3c5796817f3d5c988e30e00b5ee08a0
---
...
layer for agentic coding, as actually implemented in this repository at v0.12.0.
...
configured → `LOCAL_ONLY` message, exit 0 (`main()`, lines 494–497): a mode, not
...
|---|---|---|---|
| no | no | skip (hash equal) | `pull()`, lines 414–415 |
| no | yes | `worklog ingest` (deterministic `ev`) | lines 439–446 |
| yes | no | push (hash dirty) | `push_items()`, lines 307–318 |
| yes | yes | `worklog conflict` per changed field; never overwrite | lines 424–438 |
...
`VERSION = "0.12.0"` "kept in step with plugin/.claude-plugin/plugin.json by
...
**`sync_dispatch.Dispatcher`** (lines 134–473). Constructor:
...
|---|---|---|
| Sync outcome | `sync report: created=… conflicts=… deferred=…` + drift bullet list | `Dispatcher.report()`, lines 467–473 |
...
2. **Claude Code plugin** — `plugin/` packages skills, `/worklog:*` commands,
   ...
   copies; version `0.12.0` in `plugin/.claude-plugin/plugin.json` locked to
...
- **Sync invariants without network**: `test_dispatch.py` (idempotent double
  ...
  (level/kind/milestone, v0.12.0), schema-mirror equality, orphan
...
| Labels don't sync | pull diffs `INGEST_FIELDS` only; "Labels sync via add/del is future work" (`pull()` comment, lines 417–419) | confirmed | low | planned work |
| Remote-origin tickets | pull reports, never creates local items (`pull()`, lines 402–407) | confirmed | low | future work, kept read-safe deliberately |
...
1. **Label sync via add/del** on pull (marked future work in `pull()`,
   lines 417–419).
2. **Remote-origin ticket creation** on pull (drift-note today, lines 402–407).
