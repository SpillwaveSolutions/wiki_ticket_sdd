...
703–716). Order inside `Dispatcher.sync()` (lines 452–465): capabilities gate,
...
**Push scope** (`push_items()`, lines 295–369): open ∪ hash-dirty ∪ `--keys`.
...
orphans and titleless items are drift, never pushes (lines 299–304). On a
...
```python
                ...
                self.worklog(*link)
```
— `bin/sync_dispatch.py — push_items(), lines 359–365`

— a subprocess back into `bin/worklog`, preserving invariant 15.4 (nothing but
`worklog` writes the log).

**Pull** (`pull()`, lines 373–448): NDJSON lines from the adapter; echo
suppression by comparing `canonical_hash(line)` to `last_pushed_hash` (lines
414–415); a field-diff against the outbound shape decides remote-only vs
...
```
— `bin/sync_dispatch.py — pull(), lines 440–442`
...
overwrites (lines 430–438, "record, never overwrite under the default report
...
*mode*: `main()` prints `LOCAL_ONLY` and returns 0 (lines 494–497).
...
1. **Spec §10.5 sync surface ≠ shipped CLI.** The spec documents
   ...
   `sync_dispatch.py` lines 476–489). The shipped scope rule (open ∪ hash-dirty
...
6. **Labels don't pull** — marked future work in code (`pull()` comment, lines
   417–419); pushes carry them (GitHub adapter maps to labels), pulls ignore.
7. **Remote-origin tickets are reported, never created locally** (`pull()`,
   lines 402–407) — "report, don't act, keep this run read-safe". Deliberate,
...
`ee38438fec2f855836e65c9a95979c321efcc3d2` (tag v0.12.0); all citations are to
that tree.
