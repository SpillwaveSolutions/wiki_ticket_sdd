---
wiki_key: design/2026-07-28_v0.17.1-release-design-doc
doc_type: design
truth_state: snapshot
date: 2026-07-28
name: v0.17.1-release
tag: v0.17.1
git_hash: b71e5e423a60f5c95b43f380165af94b2cd07010
branch: main
roadmap_snapshot: docs/roadmap/2026-07-28_v0.17.1-release.md
merged_in: 6e8fce98bbfb828a4a558d4473538ed0dbee64d1
---
...
layer for agentic coding, as actually implemented in this repository at v0.17.1.
...
Major components: the `worklog` CLI (the API, 1206 lines), `fold.py` (state
...
|---|---|---|---|---|---|---|
| `bin/worklog` | CLI (1206 lines) | All log writes; every subcommand | argv, stdin (status prose, plan drafts) | log events, docs, stdout | `ulid`, `fold`, lazy others | no writes possible |
...
`--version` (`worklog 0.17.1`, from `VERSION` at `bin/worklog` line 32, held in
...
| `link <item>` | record external identity | `--system --key` required; `--url --rev --hash` | 1 link event | no match / ambiguous prefix |
...
2. **Claude Code plugin** — `plugin/` packages skills, `/worklog:*` commands,
   ...
   `0.17.1` in `plugin/.claude-plugin/plugin.json` locked to `bin/worklog VERSION`
   by `tests/test_plugin.py`. MIT LICENSE shipped. **Drift (Confirmed):**
   `README.md`'s repo-layout table row still reads "v0.16.0" against a
   `0.17.1` VERSION — now two releases stale, the same recurring cosmetic gap
   noted at v0.14.0 through v0.17.0; tracked in the code walkthrough's Gaps
   section rather than fixed here (this document is generated, not
   hand-patched — `readme` is a separate `release.sync_docs` target).
...
20 stdlib-`unittest` suites, no third-party test dependencies, run as
...
`coverage.process_startup()` in a `.pth`), target 95%. 19 → 20 suites at
v0.17.1, the first new suite since v0.15.0: `tests/test_resolve.py` ships with
the `_resolve()` fix, per the CLAUDE.md rule that new logic ships with tests.
...
(5) of the two correctness bugs found while building artifact pages, ID-prefix
resolution (#123) is fixed in v0.17.1 and banner mislabeling (#137) — cosmetic —
remains open; the phantom items the old prefix path already wrote are still in
the log (§31).
...
should sweep the two pre-existing orphan items.
