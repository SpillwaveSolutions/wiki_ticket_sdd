...
git_hash: b2bf3cf83310962a98b42eae642b2667e1bf4d49
branch: main
tag: v0.20.0
...
| `bin/ia_render.py` | Reader plane: Home, Sidebar, indexes, publish-manifest, aliases, artifact pages (v0.14.0), (v0.19.0) the wiki-flavor seam + per-`doc_type` banners + live PR pages, and (v0.20.0) `PLAN_STATE`/`_plan_state()` — a plan's banner names its own state (763 lines). |
| `bin/ia_graph.py` | Traceability graph, link-pr, ticket-body, trace-check, `build_adjacency()`/`item_links()` (v0.14.0), (v0.19.0) `pr_sync()` + the `find` search surface, and (v0.20.0) `in_trace_scope()` — the evidence gate's scope, finally enforced (543 lines). |
...
| `tests/` | 40 stdlib-unittest suites, 538 test functions; the executable spec. Merge safety alone accounts for `test_watermark.py` and `test_bug_merge.py`; `test_trace_scope.py` (v0.20.0) is the newest file. |
...
```
— `bin/worklog — check_taxonomy(), lines 108–121`
...
def append(event):
    ...
    return event
```
— `bin/worklog — append(), lines 59–84`
...
2048` (line 32) is "derived from PIPE_BUF … Not a setting." `VERSION = "0.20.0"`
...
```
— `bin/fold.py — dedupe_and_sort(), lines 163–183`
...
def position(ev):
    ...
    return (ev["ev"], 1)
```
— `bin/fold.py — position(), lines 185–208`
...
`cmd_plan_capture()` (`bin/worklog`, lines 528–595). It parses the draft with
...
(`ia_graph.ticket_body()`, lines 302–357) for the issue-description skill to push.
...
def cmd_ia_normalize(a):
    ...
    ...
```
— `bin/worklog — cmd_ia_normalize(), lines 718–729`
...
def cmd_ia_index(a):
    ...
    for path in ia_render.write_all():
        print("wrote: " + path)
```
— `bin/worklog — cmd_ia_index(), lines 742–750`
...
def link_pr(ulid_, pr=None, commit=None):
    ...
```
— `bin/ia_graph.py — link_pr(), lines 153–183`
...
def _resolve(item):
    ...
    return match[0]
```
— `bin/worklog — _resolve(), lines 204–224`
...
```
— `bin/fold.py — external_owners(), lines 97–124` (docstring elided)
...
**Enforcement 1 — write time** (`bin/worklog — cmd_link(), lines 281–313`). The
...
**Enforcement 3 — the repair** (`bin/worklog — cmd_unlink(), lines 315–347`).
...
```
— `adapters/github/adapter — create_issue(), lines 91–112`
...
```python
    ...
    ...
        ...
        if limit is None or e["ev"] > limit:
            kept.append(e)
```
— `bin/fold.py — apply_watermark(), lines 210–267`
...
**`worklog merge-rescue`** (`bin/compact.py — merge_rescue(), lines 345–455`) is
...
```
— `bin/ia_graph.py — in_trace_scope(), lines 250–267` (docstring elided)
...
```
— `bin/ia_graph.py — trace_check(), lines 269–300`
...
```
— `bin/ia_render.py — PLAN_STATE, _plan_state(), lines 109–130` (docstring
elided)
...
```
— `bin/ia_render.py — _banner_text(), lines 132–181`
...
row reads "**v0.20.0**" and matches `VERSION = "0.20.0"` in `bin/worklog` at this
commit — the recurring cosmetic gap reported at v0.14.0 through v0.17.1 has now
stayed closed for four releases. The underlying cause remains unautomated:
...
`b2bf3cf83310962a98b42eae642b2667e1bf4d49` (tag v0.20.0 on main); all
citations are to that tree, and every `lines N–M` in this document was
re-derived from that tree rather than carried forward (see "New in v0.20.0"
item 2). Dated freeze pairs for this release pin `git_hash` to this same
commit.
