...
|---|---|
| `bin/worklog` | The CLI and the **only** writer of `.work/*.jsonl` (964 lines). |
...
Entry: argparse dispatches to `cmd_add` (`bin/worklog`, lines 828–840 wiring,
...
`cmd_plan_capture()` (`bin/worklog`, lines 328–368). It parses the draft with
...
```
— `bin/worklog — cmd_plan_capture(), lines 336–338`

Then it appends the epic create, then each task/subtask create (subtasks parent
to `last_task`, lines 348–361), and finally writes the plan doc with front
...
candidate" (line 341 comment). The whole flow is *forced* by
...
`render()`'s default (comments at `bin/worklog` lines 304–306 and
...
703–716). Order inside `Dispatcher.sync()` (lines 464–477): capabilities gate,
push, pull, save state, report.

**The gate runs first, every run** (lines 198–209): adapter `capabilities`
output is parsed, validated against the embedded schema mirror
(`CAPABILITIES_SCHEMA`, lines 35–66) by a 28-line mini JSON Schema validator
(`validate()`, lines 84–111), plus one check the schema subset cannot express:

```python
        if "{ulid}" not in caps["marker"]["template"]:
            raise ContractError("$.marker.template: must contain '{ulid}'")
```
— `bin/sync_dispatch.py — capabilities(), lines 207–208`

**Push scope** (`push_items()`, lines 295–381): open ∪ hash-dirty ∪ `--keys`.
...
(`outbound()` docstring, lines 214–217). A closed item that never went remote is
inert (lines 310–314: "pushing it would file tickets for long-dead work");
orphans and titleless items are drift, never pushes (lines 299–304). A closing
item whose hash is dirty pushes an `update` with the final item shape *before*
the `close` verb (lines 332–343, new in v0.12.1): adapter `close` is
key+resolution only, so a reclassify-then-close used to leave stale remote
taxonomy labels that the next pull re-ingested over the local edit (worklog
01KY129S, GitHub #76) — "Push the final shape first; the close echo then
hash-suppresses" (comment, lines 333–337). On a
...
```python
                ...
                self.worklog(*link)
```
— `bin/sync_dispatch.py — push_items(), lines 371–377`

— a subprocess back into `bin/worklog`, preserving invariant 15.4 (nothing but
`worklog` writes the log).

**Pull** (`pull()`, lines 385–460): NDJSON lines from the adapter; echo
suppression by comparing `canonical_hash(line)` to `last_pushed_hash` (lines
426–427); a field-diff against the outbound shape decides remote-only vs
...
```
— `bin/sync_dispatch.py — pull(), lines 452–454`
...
overwrites (lines 442–450, "record, never overwrite under the default report
policy" — and since v0.12.0 `report` is the *only* policy; see §6). The
field-diff runs over `INGEST_FIELDS` (lines 27–28), which since v0.12.0
includes `level`/`kind`/`milestone`, so remote taxonomy edits pull instead of
silently dropping.

**Failure handling** is an exit-code table, not exceptions (`handle_exit()`,
lines 246–269): 2 aborts the sync after saving state; 3 pops
`last_pushed_hash` so the item re-pushes next run; 4 was already retried with
doubling backoff (`call_push()`, lines 233–244); 5 fetches the remote and files
per-field conflicts; anything else is a drift note. No adapter at all is a
*mode*: `main()` prints `LOCAL_ONLY` and returns 0 (lines 506–509).
...
`_status_facts()` (`bin/worklog`, lines 545–665) is the deterministic half: a
fold plus a raw-event pass; an event is in-window when its **`ev` ULID
timestamp** is (lines 559–560). Daily windows open at the last daily report's
date; weekly is a fixed 7 days; timecards bucket per UTC day, attach best-effort
git commit subjects (`_git_commits()`, lines 503–515: any git failure → `[]`),
and carry no item IDs — "narrative only" (lines 569–571). The prose is
explicitly not this code's job: "The prose is the skill's job, not ours" (line
548). `cmd_status --write` stamps front matter with the window and the
`through` watermark and refuses to overwrite without `--force` (lines 693–696) —
...
| 2 | Only `worklog` writes the log; only `compact.py` rewrites it | policy + CLAUDE.md; `sync_dispatch` shells into `worklog` (lines 178–188) | hand edits corrupt merges; invariants unauditable |
...
| 4 | Ingested events carry deterministic `ev` and the remote's `ts` | `ulid.deterministic()`; `cmd_ingest()` lines 184–187 | duplicate ingests survive dedupe and silently revert local edits (`test_random_ev_silently_reverts_the_local_edit`) |
| 5 | Push idempotency: marker `worklog:<ulid>` + canonical-hash skip | `push_items()` lines 307–317; marker template gate lines 207–208 | retried pushes file duplicate tickets |
...
**`tests/test_dispatch.py — test_pull_ingests_remote_taxonomy_change()` (lines
149–162), new in v0.12.0.** A remote edit to `level`/`kind`/`milestone` lands in
the local log on pull. Regression caught: the v0.11.0 gap where the dispatcher's
`INGEST_FIELDS` silently dropped remote taxonomy edits.

**`tests/test_dispatch.py — TestCloseSyncsFields` (lines 174–189), new in
v0.12.1.** Reclassify a synced item (`--kind bug`), close it, push, pull.
Asserts the local `kind` survives the round-trip and `ingest_events() == []` —
the pull reads as an echo, not a remote edit. Regression caught: close pushing
only key+resolution, leaving stale remote taxonomy labels that the next pull
re-ingested over the local edit (worklog 01KY129S, GitHub #76).

**`tests/test_ingest.py — TestReopen` (lines 148–172), new in v0.12.0.** Three
rules: `worklog reopen` moves a closed item to `todo` *and* drops the stale
`resolution`; `update --status` on a closed item is refused with a pointer to
`reopen` (and appends nothing); reopening an open item is refused.
...
Three v0.11.0 gaps closed in v0.12.0: the dispatcher's `INGEST_FIELDS` now
carries `level`/`kind`/`milestone` (lines 27–28 — remote taxonomy edits pull),
`worklog reopen` exists (`cmd_reopen()`, `bin/worklog` lines 146–160, with
`update --status` on closed items refused, lines 120–122), and
`conflict_policy` was descoped to `report` only — `local-wins`/`remote-wins`
were documented but never read by the dispatcher, so the config comment and
spec §10.6 now say `report` is the only policy. One v0.11.0 item was itself
wrong and is corrected here: it claimed no suite exercises `worklog resolve`'s
CLI path, but `test_ingest.py — TestResolve` (lines 118–145) does exactly that.

1. **Spec §10.5 sync surface ≠ shipped CLI.** The spec documents
   `--scope active|all`, `--report`, `--apply`; the CLI ships `--dry-run`,
   `--keys`, `--push-only`, `--pull-only` (`build_parser()`,
   `sync_dispatch.py` lines 488–501). The shipped scope rule (open ∪ hash-dirty
   ∪ keys) matches spec §10.5's *intent*; the flags don't. Doc drift, not a bug.
2. **Spec status line is stale.** The spec header says "step 10 outstanding"
   and §12 says `status-report` is "Still specified, not yet shipped", but
   `cmd_status` is fully implemented (lines 668–700) and
   `plugin/skills/status-report/` exists. The spec's changelog hasn't caught up
   with the code.
3. **`.work/config.yml` comments predate the typed contract.** Both `ticketing`
...
4. **Spec §11's three-phase orchestration (changeset.json, results/) is not in
...
5. **`estimate` field (spec §5.4) has no CLI flag** — settable only via ingest?
   No: `INGEST_FIELDS` excludes it too. Dead field today.
6. **Labels don't pull** — marked future work in code (`pull()` comment, lines
   429–431); pushes carry them (GitHub adapter maps to labels), pulls ignore.
7. **Remote-origin tickets are reported, never created locally** (`pull()`,
   lines 414–419) — "report, don't act, keep this run read-safe". Deliberate,
   but worth knowing before expecting round-trip creation.
8. **Duplicated mini-validator** (three copies: dispatcher, `adr.py`, contract
...
`04cf8e8111bd0be37f7048f72474baf75e836635` (tag v0.12.1); all citations are to
that tree.
