---
wiki_key: design/2026-07-28_v0.17.1-release-code-walkthrough
doc_type: design
...
|---|---|
| `bin/worklog` | The CLI and the **only** writer of `.work/*.jsonl` (1206 lines). |
...
`README.md`'s `plugin/` row still reads "v0.16.0" against `VERSION =
"0.17.1"` in `bin/worklog` — now **two** releases stale, the same cosmetic gap
noted at v0.14.0, v0.15.0, v0.16.0, and v0.17.0. `readme` is in
`release.sync_docs`, but that target is regenerated by a separate agent and this
document is generated, not hand-patched, so the row is reported here as observed
...
`b71e5e423a60f5c95b43f380165af94b2cd07010` (tag v0.17.1 on main); all
