---
wiki_key: design/2026-08-04_v0.21.0-release-design-doc
doc_type: design
truth_state: snapshot
date: 2026-08-04
name: v0.21.0-release
tag: v0.21.0
git_hash: "8471d1b89f90d7ba5bdc509a662173688cda079f"
branch: docs/design-sync-v0210
roadmap_snapshot: docs/roadmap/2026-08-04_v0.21.0-release.md
merged_in: 7714eacb7744749fdb15698e773f569341adcdd1
---
...
layer for agentic coding, as actually implemented in this repository at v0.21.0.
...
`docs/adr/0001..0008` (Architecture Decision Records — 0008 is new in v0.21.0 and
records what document provenance depends on), `docs/plans/` (the *why*
...
- **Documents enter git time, and a gate reads it back (v0.21.0, this release's
  centre of gravity)**: every generated document is stamped with the commit its
...
499 lines), `render_roadmap.py` (286) + `viz_mermaid.py` (180) (generated docs),
`sync_dispatch.py` (857 lines) + `adapters/` (ticket sync), `ia.py` (624) /
...
| `bin/compact.py` | Batch (CI) + merge tool (499 lines) | The only file rewriter; verified. Also the two merge guards (`merge_check`) and, since v0.19.0, **`merge_rescue()`** — the repair, reasoning from the merge base | log files, `MERGE_HEAD`/merge base | rewritten logs, guard problems, rescued events | `fold`, `ulid`, `render_roadmap.max_ev`, `git` | worst-case: refused, files untouched |
| `bin/sync_dispatch.py` | Orchestrator (857 lines) | Every sync invariant; **overwrite preview + GONE policy (v0.19.0)** | fold output, adapter I/O | pushes, ingests, conflicts, report, overwrite block | `canonical`, `fold.external_owners`, adapter, `worklog` | sync only; local-only fallback |
...
**`worklog merge-rescue`** (`bin/compact.py — merge_rescue(), lines 345–454`) is the
...
(`bin/worklog` line 33: `VERSION = "0.21.0"`), and since v0.19.0 also
...
create_issue(), lines 91–113`, called from `adapters/github/adapter — cmd_push(), lines 243–283`).** The
...
2. **Claude Code plugin** — `plugin/` packages skills, `/worklog:*` commands,
   ...
   `0.21.0` in `plugin/.claude-plugin/plugin.json` locked to `bin/worklog VERSION`
   by `tests/test_plugin.py`. MIT LICENSE shipped. **Drift closed (Confirmed):**
   `README.md`'s repo-layout table row reads "**v0.21.0**" and matches VERSION at
...
**41 stdlib-`unittest` suites and 577 test functions** (Confirmed by count at
this commit), no
...
`doc-verify --strict` becomes a CI job (Open Question 8).
