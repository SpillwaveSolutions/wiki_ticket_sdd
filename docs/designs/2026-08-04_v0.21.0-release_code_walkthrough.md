---
wiki_key: design/2026-08-04_v0.21.0-release-code-walkthrough
doc_type: design
...
| `bin/sync_dispatch.py` | Every ticket-sync invariant, in one place (857 lines). |
| `bin/compact.py` | The only file rewriter (499 lines). CI-only for compaction; also the two merge guards and, since v0.19.0, `merge_rescue()` — the repair an operator runs by hand. |
...
| `tests/` | 41 stdlib-unittest suites, 577 test functions; the executable spec. Merge safety alone accounts for `test_watermark.py` and `test_bug_merge.py`; `test_provenance.py` (v0.21.0, 548 lines) is the newest and largest file. |
...
2048` (line 32) is "derived from PIPE_BUF … Not a setting." `VERSION = "0.21.0"`
...
```
— `adapters/github/adapter — create_issue(), lines 91–113`
...
```
— `bin/ia_render.py — _plan_state(), lines 119–139` (`PLAN_STATE` at lines
...
ADR-0008 before "fixing" an `unresolvable`).
...
`8471d1b89f90d7ba5bdc509a662173688cda079f` (tag v0.21.0), and every
...
specific defect #294 was filed for. Unlike every edition before it, that claim
is now checkable rather than merely asserted: run `bin/worklog doc-verify` and
this file should report zero fabrications and zero drift. Dated freeze pairs for
this release pin `git_hash` to the same commit.
