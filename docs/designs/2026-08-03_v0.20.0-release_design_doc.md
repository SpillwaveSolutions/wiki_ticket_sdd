---
wiki_key: design/2026-08-03_v0.20.0-release-design-doc
doc_type: design
...
branch: main
roadmap_snapshot: docs/roadmap/2026-08-03_v0.20.0-release.md
merged_in: e95ad4d83931e328fc3381170b9be157d422bfdf
---
...
layer for agentic coding, as actually implemented in this repository at v0.20.0.
...
`docs/plans/2026-08-02-trace-check-scope.md` and
`docs/plans/2026-08-02-plan-banner-state.md`, both v0.20.0, preceded by
...
`ia_render.py` (763) / `ia_graph.py` (543) (reader plane + graph + artifact
...
enforced by no CI job — deliberately deferred until the (now 16) remaining gaps
are driven to zero and held there for a release.
...
`worklog` (invariant 15.4, `sync_dispatch.py — Dispatcher.worklog(), lines 249–259`).
...
Confirmed in `cmd_plan_capture()` (`bin/worklog`, lines 528–595): indented checkboxes
...
535–540` and `report_collisions(), lines 409–432`).** Before the per-item loop,
...
**Scope now includes key-dirty (v0.18.0, `is_dirty(), lines 219–234`).**
...
alongside `last_pushed_hash` (`record_push(), lines 236–240`) and treats a change
...
**Auto-link can no longer abort a run (v0.18.0, `record_link(), lines 261–279`).**
...
`compact.py — compact(), lines 165–173` and `_snapshot(), lines 45–64`).** After
...
`_resolve()` (`bin/worklog — _resolve(), lines 204–224`) folds
...
3. **Repair** — `bin/worklog — cmd_unlink(), lines 315–347`. Prints the freed
...
| 2 | `apply_watermark()` drops per item; an item with **no** snapshot never loses an event | `fold.py — apply_watermark(), lines 210–267` | Nothing folded those events, so nothing carries their state. This is compaction's own "never drop data" rule (spec §7 step 3) applied on the read side |
| 3 | A snapshot sorts at its `through`, not its `ev` | `fold.py — position(), lines 185–208` | Puts the snapshot back after the events it folded and before anything later on any branch, so a branch's later close applies *on top of* it. Ordering had to be separated from identity — giving the snapshot the `ev` of its `through` would collide with the very event it replaced |
...
**`worklog merge-rescue`** (`compact.py — merge_rescue(), lines 345–455`) is the
...
- `bin/worklog — base(), lines 86–106` stamps `git` on every CLI-written event;
...
The rule now has its own predicate, `ia_graph.in_trace_scope()` (lines 250–267),
and `trace_check()` (lines 269–300) branches on it. Three conditions, each
...
| `ia_render.py` | `render_all`, `write_all`, `banner`, `build_manifest`, `build_aliases`; `PLAN_STATE` (v0.20.0) | check mode reports stale paths; `banner()` raises `KeyError` on a status record with no `kind` (v0.20.0) — a schema violation must not render plausible prose | `test_ia` (TestRender, TestBanner) |
...
(`bin/worklog` line 33: `VERSION = "0.20.0"`), and since v0.19.0 also
...
`--version` (`worklog 0.20.0`, from `VERSION` at `bin/worklog` line 33, held in
...
create_issue(), lines 91–112`, called from `cmd_push(), lines 243–282`).** The
...
2. **Claude Code plugin** — `plugin/` packages skills, `/worklog:*` commands,
   ...
   `0.20.0` in `plugin/.claude-plugin/plugin.json` locked to `bin/worklog VERSION`
   by `tests/test_plugin.py`. MIT LICENSE shipped. **Drift closed (Confirmed):**
   `README.md`'s repo-layout table row reads "**v0.20.0**" and matches VERSION at
   this commit — as it has since v0.18.0. New in v0.20.0: `CLAUDE.md` and its
...
**40 stdlib-`unittest` suites and 538 test functions** (Confirmed by count at
...
39 → 40 at v0.20.0 (`tests/test_trace_scope.py`); 20 → 21 at v0.18.0
(`tests/test_github_adapter.py`), 19 → 20 at v0.17.1 (`tests/test_resolve.py`).
...
| Design-doc code citations drift silently | `lines N–M` citations are prose; nothing regenerates or checks them, so they rot as the files they point into grow. #294 was filed for a stale merge tiebreak, a version constant three releases old, and an import `fold.py` does not have; the v0.20.0 pass found ~12 more offsets into `bin/worklog` and `bin/sync_dispatch.py` still stale in the *corrected* v0.19.1 pair | confirmed | medium | every offset in the current pair was re-derived at this tag. The durable fixes are to cite `file — function()` without offsets, or to generate them; neither is done (#294) |
...
remote-origin creation) rather than correctness, and none of it blocks a
release.
...
The same shape is what makes the stale line citations in §31 worth tracking:
prose that reads as verified and is checked by nothing.
