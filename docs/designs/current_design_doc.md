---
generated_at: 2026-07-21T15:03:44Z
git_hash: 04cf8e8111bd0be37f7048f72474baf75e836635
branch: main
tag: v0.12.1
...
layer for agentic coding, as actually implemented in this repository at v0.12.1.
...
| No YAML library | Stdlib-only; naive block scans where needed | `bin/worklog — _config_system(), lines 719–738`; `merge-when-green.sh` awk scan |
| Embedded schemas mirrored from `schema/` | Installed repos ship `bin/` without `schema/`; tests assert the copies match | `sync_dispatch.py — CAPABILITIES_SCHEMA, lines 35–66`; `tests/test_dispatch.py — test_embedded_capabilities_schema_matches_file()` |
...
|---|---|---|---|---|---|---|
| `bin/worklog` | CLI (964 lines) | All log writes; every subcommand | argv, stdin (status prose, plan drafts) | log events, docs, stdout | `ulid`, `fold`, lazy others | no writes possible |
...
Confirmed in `cmd_plan_capture()` (lines 328–368): indented checkboxes become
subtasks parented to the preceding task (`last_task`, lines 348–361); captured
items get explicit `kind:feature` ("a plan is deliberate feature work", line 341
...
Failure flows (Confirmed, `handle_exit()`, lines 246–269): exit 2 aborts the whole
...
configured → `LOCAL_ONLY` message, exit 0 (`main()`, lines 506–509): a mode, not
an error. Orphan/titleless items are never pushed (`push_items()`, lines 299–304).
...
(`push_items()`, lines 332–343). Adapter `close` is key+resolution only, so a
reclassify-then-close previously left stale remote taxonomy labels that the
next pull re-ingested over the local edit (worklog 01KY129S, GitHub #76);
the update's echo then hash-suppresses on pull. Regression test:
`tests/test_dispatch.py — TestCloseSyncsFields`.
...
`cmd_status()` (lines 668–700): `--emit-facts` prints deterministic JSON facts
(windowed by ULID timestamps, `_status_window()` lines 518–542); the
...
best-effort git commit subjects (`_git_commits()`, lines 503–515).
...
three open statuses (`cmd_update` choices, line 844); the diagram shows the
intended flow. Closed→open goes only through `worklog reopen` (new in v0.12.0,
`cmd_reopen()`, lines 146–160): `update --status` on a closed item is refused
with a pointer to `reopen`, because only the `reopen` op also drops the stale
`resolution` (`cmd_update()`, lines 120–122; fold rule 6 above). Compaction
relocates closed items physically; `close` itself never moves files (spec §7).

## 9.3 Conflict lifecycle decision table

| Local changed since last push | Remote changed | Dispatcher action | Cited |
|---|---|---|---|
| no | no | skip (hash equal) | `pull()`, lines 426–427 |
| no | yes | `worklog ingest` (deterministic `ev`) | lines 451–458 |
| yes | no | push (hash dirty) | `push_items()`, lines 307–318 |
| yes | yes | `worklog conflict` per changed field; never overwrite | lines 436–450 |

Resolution: `worklog resolve <item> --field F --take local|remote` appends a plain
`update` that outsorts the conflict (`cmd_resolve()`, lines 227–244).
...
`VERSION = "0.12.1"` "kept in step with plugin/.claude-plugin/plugin.json by
...
**`sync_dispatch.Dispatcher`** (lines 134–485). Constructor:
...
substring check the mini-validator cannot express, lines 198–209), `sync()`
(capabilities → push → pull → save state → report, lines 464–477),
`push_items()`, `pull()`, `report()`. State managed: per-item `last_pushed_hash`, per-system `cursors`,
...
lines 155–160). Exceptions: `ContractError` — caught in `main()` and reported as
exit 1.

**`sync_dispatch.ContractError`** (lines 69–70). "An adapter broke the typed
...
| `update <item>` | mutate | `--status todo\|in_progress\|blocked`, `--priority`, `--title`, `--kind`, `--milestone`, `--add-label`, `--del-label` | 1 update event | "nothing to update"; taxonomy re-checked against current level (lines 116–119); `--status` refused on closed items with a pointer to `reopen` (lines 120–122) |
| `close <item>` | close | `--status done\|cancelled`, `--resolution` | 1 close event | — |
| `reopen <item>` | reopen a closed item (v0.12.0) | prefix match on the id | 1 reopen event — fold moves status to `todo` and drops the stale `resolution` (`cmd_reopen()`, lines 146–160) | not closed → "nothing to reopen"; no match |
| `link <item>` | record external identity | `--system --key` required; `--url --rev --hash` | 1 link event | — |
| `ingest <item>` | ingest remote change | `--system --key --rev --rev-ts-ms` required; `--set FIELD=VALUE`, label flags | 1 update event, deterministic `ev`, `ts` = remote clock | field/enum whitelist (lines 193–204) |
...
hash-dirty ∪ `--keys`). See §34. The v0.11.0 gap here — a fold-supported
`reopen` op with no CLI — closed in v0.12.0 (`cmd_reopen()`, lines 146–160).
...
(`call_push()`, lines 233–244). Idempotency: marker `worklog:<ulid>` embedded in
...
`task`, with a drift note (`outbound()`, lines 213–229).
...
- **Contract errors**: `ContractError` names the offending field path
  (`validate()`, lines 84–111); the capabilities gate runs "first, every run,
  before any push" (line 195 comment).

Degradation ladder: no adapter → local-only, exit 0; adapter lacking `pull` →
drift note "local log may lag remote" (`pull()`, lines 374–376); unsupported
...
|---|---|---|
| Sync outcome | `sync report: created=… conflicts=… deferred=…` + drift bullet list | `Dispatcher.report()`, lines 479–485 |
...
| Work-in-flight age | status report `in_progress` age_days | `_status_facts()`, lines 632–641 |
| Unplanned-work ratio | status "unplanned_in_window" with `discovered_during` attribution | `_status_facts()`, lines 623–629 |
...
the only policy since v0.12.0 descoped `local-wins`/`remote-wins` auto-resolve,
...
2. **Claude Code plugin** — `plugin/` packages skills, `/worklog:*` commands,
   ...
   copies; version `0.12.1` in `plugin/.claude-plugin/plugin.json` locked to
...
- **Sync invariants without network**: `test_dispatch.py` (idempotent double
  ...
  echo suppression, deterministic pull `ev`, remote taxonomy ingest
  (level/kind/milestone, v0.12.0), dirty-close update-before-close
  (v0.12.1), schema-mirror equality, orphan
...
- **The rest**: taxonomy write rules, ingest whitelist, reopen (clears stale
  resolution; `update --status` on closed refused; reopen of open refused),
  status facts, roadmap
...
throwaway state dir, `_fake_round_trip()`, lines 741–770). Common failure: a
...
| Labels don't sync | pull diffs `INGEST_FIELDS` only; "Labels sync via add/del is future work" (`pull()` comment, lines 429–431) | confirmed | low | planned work |
| Remote-origin tickets | pull reports, never creates local items (`pull()`, lines 414–419) | confirmed | low | future work, kept read-safe deliberately |
| Triple-copy mini-validator | `sync_dispatch`, `adr`, tests | — | low | deliberate; extract on the fourth copy |
| Spec/CLI drift on sync scopes | §10.5 flags (`--scope/--report/--apply`) lack CLI surface | confirmed | low | §34 |

(The v0.11.0 top risk — dispatcher pull fields lagging the taxonomy — closed in
v0.12.0: `INGEST_FIELDS` now carries `level/kind/milestone`, `sync_dispatch.py`
lines 27–28, proven by `test_pull_ingests_remote_taxonomy_change`.)

# 32. Implementation Plan (extension roadmap)

Built system; recommended next steps in order (Recommendation, grounded in
in-code TODO markers and spec §18). v0.12.0 shipped the previous list's top two
(dispatcher taxonomy ingest; `worklog reopen`); what remains:

1. **Label sync via add/del** on pull (marked future work in `pull()`,
   lines 429–431).
2. **Remote-origin ticket creation** on pull (drift-note today, lines 414–419).
3. **Spec §17 open questions** that gate features: multi-repo aggregation (Q5),
   superseded-plan unpublishing (Q7).

Critical path: none — 1 and 2 both touch the pull diff but are independent;
everything parallel.
...
(v0.11.0's question 6 — a `reopen` CLI — shipped in v0.12.0; its question about
`conflict_policy` auto-resolve was answered by descoping to `report` only.)
...
place state loss is possible; (3) labels and remote-origin tickets still don't
pull (deliberate read-safety, but a visible sync asymmetry). The v0.11.0 risk
of dispatcher pull fields lagging the taxonomy closed in v0.12.0.
...
**Recommended implementation order:** §32 items 1→2→3.

**Information still needed from stakeholders:** multi-repo requirements (Open
Question 3). The v0.11.0 question of whether any team needs
`conflict_policy: local-wins|remote-wins` was answered in v0.12.0 by descoping:
`report` is the only policy (config comment and spec §10.6 now say so), and
auto-resolve returns only via a new plan, not a config value.
