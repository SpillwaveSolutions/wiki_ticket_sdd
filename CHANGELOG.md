...
- **CI can own ticket sync (#413).** `ticketing.sync_owner: ci` makes `worklog-post-merge` the syncer: it names the adapter (`WORKLOG_TICKET_ADAPTER`, since a runner cannot discover it), caches `.work/sync-state.json`, runs `worklog sync --push-only --force` with the bot token before rendering, and stages `.work/todo.jsonl` with the docs so the link events ride the derived-docs PR. A manual push sync in a CI-owned repo exits 1 without `--force`; `--report`, `--explain`, and `--pull-only` stay open; `session-doctor` notes the ownership. `ticketing.ci_dedupe_check: true` adds `worklog dedupe --dry-run --check` to every PR run (new `--check`: exit 1 on an agreed duplicate group), skipped when the token is absent. New helper `sync_dispatch.ticketing_config()`. This repository stays `sync_owner: human`.

- **Bot PRs use a maintainer PAT; the status bridge is gone (ADR-0011).** `worklog-compact` and `worklog-post-merge` open and merge their PRs with the `WORKLOG_BOT_PAT` secret (fine-grained, contents + pull-requests + issues read/write, one-year expiry; secret created 2026-09-19 with a one-year expiry, so rotate before 2026-09-19 of next year). `pull_request` and `push` checks now run natively on bot commits, so `plugin/scripts/associate-pr-checks.sh`, the `workflow_dispatch` wait, `actions: write`, `statuses: write`, and the `workflow_run` listener in `worklog-invariants` are deleted. Both bot workflows declare `permissions: contents: read` and fail at their first step when the secret is missing. Under the strict up-to-date policy a second open bot PR goes stale the moment the first merges, so each bot job closes every open `chore/compact-*` and `chore/post-merge-*` PR before regenerating from `main` (supersede, not rebase), and merges with `--delete-branch`. Post-merge skips its own PRs by branch prefix, which with a real identity is required rather than accidental. The `sync --report` step is no longer masked by `continue-on-error`; a drift finding reaches the PR comment, and a repo with no adapter gets no comment at all. An eviction-only compaction is titled `compact: evict to archive` instead of repeating last night's watermark.

This closes the plan `docs/plans/2026-09-19-review-v0-24-10-and-open-tickets.md` (epic #414): every workstream from the v0.24.10 review and issues #412 and #413 is merged. Live design docs regenerate after the tag through the design-docs skill in release mode; the freshness gate blocks the next release if that PR does not land.

## 0.24.11 — 2026-09-19

- **Release gate for design-doc freshness.** `doc-verify --strict` (repo-wide runs only, never the `--staged` hook) now reports `STALE` and fails when a live design doc's `git_hash` does not descend from the latest tag or when that tag has no freeze record in `docs/designs/` (either `<date>_<tag>-release.md` or the historical dated pair). Seven releases (v0.24.4 to v0.24.10) shipped with the live pair stamped at v0.24.3 because the regeneration step was prose. The release skill now spells out the `Agent` call and a completion check, and the live pair plus a freeze note for v0.24.10 land with this change.

- **`worklog init` writes `azure-pipelines.yml` for an Azure DevOps origin (#413).** Forge is detected from the origin remote (`dev.azure.com`, `*.visualstudio.com`); every other host keeps the GitHub Actions file. Same hook-only steps, existing file never overwritten, `uninstall` removes it. A contract test keeps the two templates' commands identical. The user guide gains a "CI wiring" section with the GitHub Actions to Azure Pipelines mapping table. Adapter and CI host are separate axes; no renderer, no forge override key.
...
- **`sync` no longer mints a duplicate ticket on a clone with no link memory (#412).** `observe_remote` already listed every remote ticket; it now keeps the marker-to-key map and `remembered_key` consults it as its third source after the log's `external.key` and the per-clone `last_pushed_key`, so the push is an update and the relink path records the missing link event with no extra API call. Colliding markers resolve to the `dedupe` survivor. A probe hit that is closed on the remote closes locally instead of pushing. When the listing fails and the clone has no push memory, creates are skipped for that run and the report says why. A deliberate `worklog unlink` is never undone by the probe. New: `worklog sync --explain <ULID>` prints which source answered and changes nothing; the report counts `relinked`, and a run that created tickets prints the `dedupe --dry-run` hint; `worklog adapter check` says when the clone has no push memory yet. Residual causes stay documented on `remembered_key`: a capped or lagging listing, or two syncers that both observe absence.
...
Live design docs for this release regenerate after the tag through the design-docs skill in release mode, and the freshness gate in `doc-verify --strict` blocks the next release if that PR does not land. The v0.24.10 pair and its freeze note landed in #440 with this release's changes. Bot-PR identity (#437) and CI-owned sync (#438) are complete but wait on the `WORKLOG_BOT_PAT` secret; they ship in the next release.
