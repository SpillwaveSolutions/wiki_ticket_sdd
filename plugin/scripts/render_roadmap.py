...
def top_event(paths):
    """Newest event across the input files; None on an empty log.

    Returns the whole event, not just its id, because the roadmap needs two
    facts from it: the ULID for `generated_at` and the `git` field for
    `git_hash`. Both must come from committed bytes — see render().
    """
    top = None
    for path in paths:
        try:
        ...
        with fh:
            for line in fh:
                ...
                try:
                    ...
                    continue
                if isinstance(e, str) and (top is None or e > top.get("ev")):
                    top = ev
    return top
...
def max_ev(paths):
    ...
    top = top_event(paths)
    return top.get("ev") if top else None
...
def render(paths=PATHS, viz="deps,hierarchy"):
    ...
    gen = (time.strftime("%Y-%m-%dT%H:%M:%SZ",
                         time.gmtime(ulid.timestamp_ms(top["ev"]) / 1000))
           if top else "never")
    ...
    # the next run -- failing every commit thereafter. On pull_request CI it
    # is worse: the checkout is a synthetic refs/pull/N/merge sha that exists
    # in no local clone, so no stored value could ever match. Omitted when
    # absent (pre-0.19.1 logs, or no git), never written empty -- an empty
    # value opens a block list in ia.parse_front_matter and swallows the
    # lines after it.
    ...
    lines = [
        ...
        f"generated_at: {gen}",
        # Quoted, and that is not cosmetic. Events carry the SHORT sha, and
        # ia._scalar coerces an all-digit value to int before it considers
        # quotes -- so roughly one short sha in 27 would be read back as an
        # integer, and one with a leading zero would come back corrupted but
        # still sha-shaped. Quoting reaches the quote-strip branch first.
    ] + ([f'git_hash: "{commit}"'] if commit else []) + [
        "---",
        "",
